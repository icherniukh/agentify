# Scout Agent - Claude Code Ecosystem Discovery Specialist
...
**Purpose:** Scout searches the Claude Code ecosystem for existing plugins, agents, skills, MCP servers, and conventions that match a stated problem or need. It evaluates fitness, presents findings with actionable recommendations, and helps developers leverage existing community solutions.

---

## Complete Agent File

**File location:** `~/.claude/agents/scout.md`
...
Scout needs access to:

1. **WebSearch** - Search GitHub, marketplaces, community resources
2. **WebFetch** - Retrieve README files, documentation, marketplace catalogs
3. **Read** - Check local .claude/ directory for already-installed solutions
4. **Grep** - Search local skills/agents for related functionality
5. **Glob** - Discover installed plugins/skills that may already address the need

**Rationale:**
- Web tools enable ecosystem discovery
- Local tools prevent recommending what user already has
- No Write/Edit needed (Scout only recommends, doesn't install)
...
1. **Create agent file:**
   ...
   # Copy the complete agent markdown (from --- to end) to:
   ~/.claude/agents/scout.md
   ```

2. **Verify installation:**
   - Restart Claude Code or reload window
   - Scout should appear in agent list
   - Test with: "Scout, what MCP servers exist for database access?"

3. **Optional: Create slash command:**
   ```bash
   mkdir -p ~/.claude/slash-commands
   echo "Invoke Scout agent to search Claude Code ecosystem for existing solutions to the stated problem." > ~/.claude/slash-commands/scout.md
   ```

4. **Usage patterns:**
   - Direct invocation: "Scout, I need [problem]..."
   - Task delegation: Use Task tool with agent: "scout"
   - Slash command: `/scout [problem description]`
...
**Update triggers:**
- New major marketplace launches (add to primary sources)
- Ecosystem consolidation (update source priority)
- Search result quality degradation (refine ranking algorithm)

**Community contributions:**
- Submit new sources via PR to Scout's source list
- Share Scout reports as examples for calibration
- Report false positives/negatives for search improvement
...
- 80%+ of searches return at least one Medium+ fit solution
- Users choose existing solutions over building custom 60%+ of time
- Average time-to-solution discovery <5 minutes
- False positive rate <20% (recommended solutions actually work)

---

## Example Integration with Kim

**Workflow pattern:**

User: "Kim, I want better test coverage reporting."

Kim: "Let me delegate to Scout to see what exists in the ecosystem first."

Scout: [Searches and finds pytest-coverage-mcp and testing-dashboard-plugin]

Kim: "Based on Scout's findings, pytest-coverage-mcp is a strong fit. Shall I install and configure it, or would you like to review Scout's full report first?"

**Benefit:** Kim leverages Scout's ecosystem knowledge before building custom solutions, following "find before building" principle.

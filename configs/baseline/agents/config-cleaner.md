...
- Read both `settings.json` and `settings.local.json`. If any top-level keys appear in both, report the conflict with both values.
- Read the `mcpServers` object from `settings.json`. For each server:
...
- **Never scan plugins/, marketplaces/, or runtime directories** — those are managed externally.
